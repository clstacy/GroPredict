...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 10 12:44:56 2021
...
class KnnDtw(object):
    ...
    def _dist_matrix(self, x, y):
        ...
        if(np.array_equal(x, y)):
            ...
            p = ProgressBar(np.shape(dm)[0]) #added np to remove error
...
class ProgressBar:
    ...
    def animate_ipython(self, iter):
        print('\r', self) #changed to add parentheses and removed end comma. not sure effect
        sys.stdout.flush()
...
x_train = np.array(np.log(x_train))
...
print (classification_report(label, y_test, target_names=[l for l in np.unique(y_test)]))
...
_ = plt.xticks(range(6), [l for l in np.unique(y_train)], rotation=90)
_ = plt.yticks(range(6), [l for l in np.unique(y_train)])
